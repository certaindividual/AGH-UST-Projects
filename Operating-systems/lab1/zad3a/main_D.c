...
void fillArray(struct arrayOfBlocks* array, int blockSize) {
	void *handle = dlopen("./blockManagement.so", RTLD_LAZY);
	void (*addBlock)(struct arrayOfBlocks*, char*, int) = dlsym(handle, "addBlock");
...
void removeXBlocks(struct arrayOfBlocks* array, int x, int from){
	void *handle = dlopen("./blockManagement.so", RTLD_LAZY);
	void (*deleteBlock)(struct arrayOfBlocks*, int) = dlsym(handle, "deleteBlock");
	for(int i =from; i<from+x;i++) 
		deleteBlock(array, i);
}
...
void operation(struct arrayOfBlocks* array, char* toDo, int how, int blockSize, int from){
	...
	if(strcmp(toDo, "search_element")==0){
		void *handle = dlopen("./blockManagement.so", RTLD_LAZY);
		void (*findElemOfTheClosestASCIISum)(struct arrayOfBlocks*, int) = dlsym(handle, "findElemOfTheClosestASCIISum");
...
int main(int nrOfArgs, char **arg) {
	...
	void *handle = dlopen("./blockManagement.so", RTLD_LAZY);
	struct arrayOfBlocks* (*createArray)(int , int) = dlsym(handle, "createArray");
